...
export default function AnimeEpisodeActions({
		...
		if (isEpisodeWatched) {
			form.delete(route('anime.episode.destroy'), {
				...
				onSuccess: (res: any) => {
					if (res.props.flash.success) {
						notifications.show({
							title: 'Success',
							message:
								res.props.flash.message || 'Episode removed from your library',
							color: 'green',
						});
					}
					if (!res.props.flash.success) {
				...
				onError: (errors: FormErrors) => {
					notifications.show({
						title: 'Error',
						message: errors.message || 'Failed to remove episode from library',
						color: 'red',
					});
				},
			});
			...
			form.post(route('anime.episode.store'), {
				...
				onSuccess: (res: any) => {
					if (res.props.flash.success) {
						notifications.show({
							title: 'Success',
							message:
								res.props.flash.message || 'Episode added to your library',
							color: 'green',
						});
					}
					if (!res.props.flash.success) {
						notifications.show({
							title: 'Error',
							message:
								res.props.flash.message || 'Failed to add episode to library',
							color: 'red',
						});
					}
				},
				onError: (errors: FormErrors) => {
					notifications.show({
						title: 'Error',
						message: errors.message || 'Failed to add episode to library',
						color: 'red',
					});
				},
			});
	...
	return (
		<Button
			...
			p={4}
			variant={
				isEpisodeWatched ? (isHovered ? 'outline' : 'filled') : 'outline'
			}
