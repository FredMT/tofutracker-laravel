...
import { EpisodeCountdown } from '@/Components/Content/Shared/Regular/EpisodeCountdown';
			...
			>
				...
				>
					{Number(data.rating) > 0 && (
						<Badge variant='outline'>{Number(data.rating).toFixed(2)}</Badge>
					)}
					{data.type && <Text>{data.type}</Text>}
					{data.startdate && <Text>{data.startdate}</Text>}

					{Object.keys(data.mapped_episodes.mainEpisodes).length > 0 && (
						<Text>
							{Object.keys(data.mapped_episodes.mainEpisodes).length} Episodes
						</Text>
					)}
					{data.total_runtime && <Text>{data.total_runtime}</Text>}
					{countdown && <EpisodeCountdown timestamp={countdown} />}
				</Group>
			</Paper>
		);
	}
		...
		>
			...
			>
				...
				{data.tmdbData.data.original_language && (
					<Text>{getLanguageName(data.tmdbData.data.original_language)}</Text>
				)}
