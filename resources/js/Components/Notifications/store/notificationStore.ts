...
export const useNotificationStore = create<NotificationStore>((set, get) => ({
	...
	setNotifications: (notifications) => {
		try {
			...
			const uniqueNotifications = removeDuplicates(normalizedNotifications);
	...
	subscribeToNotifications: (userId: number) => {
		try {
			...
			channel.notification((websocketNotification: WebsocketNotification) => {
				try {
					...
					const normalizedNotification = {
						...websocketNotification,
						id: websocketNotification.id || `temp-${Date.now()}`,
						type: websocketNotification.type || 'unknown',
						read_at: null,
						created_at:
							websocketNotification.created_at || new Date().toISOString(),
					};
