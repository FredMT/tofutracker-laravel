...
export const useCommentStore = create<CommentStore>((set) => ({
	...
	addComment: async (
		...
		const tempComment = {
			...
			avatar: avatar
				? `/storage/${avatar}`
				: `https://api.dicebear.com/9.x/open-peeps/svg?seed=tofutracker-${username}`,
		};
		...
		try {
			...
			set((state) => {
				...
				// Update reply
				const updateReplyInComments = (comments: Comment[]): Comment[] => {
					return comments.map((comment) => {
						if (comment.id === parentId) {
							return {
								...comment,
								children: (comment.children || []).map((child) =>
									child.id === tempId ? response.data.comment : child
								),
							};
						}
						if (comment.children) {
							return {
								...comment,
								children: updateReplyInComments(comment.children),
							};
						}
						return comment;
					});
			...
			set((state) => {
				...
				// Remove reply
				const removeReplyFromComments = (comments: Comment[]): Comment[] => {
					return comments.map((comment) => {
						if (comment.id === parentId) {
							return {
								...comment,
						...
						if (comment.children) {
							return {
								...comment,
								children: removeReplyFromComments(comment.children),
							};
						}
						return comment;
					});
	...
	editComment: async (commentId: string, content: string) => {
		...
		try {
			...
			set((state) => {
				const revertCommentInTree = (comments: Comment[]): Comment[] => {
					return comments.map((comment) => {
						if (comment.id === commentId) {
							return {
								...comment,
	...
	vote: async (commentId: string, direction: 'up' | 'down' | null) => {
		try {
			const response = await axios.post('/votes', {
				commentId,
				direction: direction === 'up' ? 1 : direction === 'down' ? -1 : 0,
			});

			const serverDirection = response.data.direction;
			const newDirection =
				serverDirection === 1 ? 'up' : serverDirection === -1 ? 'down' : null;
					...
					return comments.map((comment) => {
						if (comment.id === commentId) {
							...
							const pointsDiff =
								...
								(newDirection === 'up' ? 1 : newDirection === 'down' ? -1 : 0);
